...
func localizedGuide() -> [GuideCopy] {
    ...
    if zh { return [
        ...
        GuideCopy(title:"讓 Tool 取得精確裝置",body:"在 Codex task 說「Use simulator-manager for this project.」。Tool 會排隊、啟動指定裝置、執行測試，並在成功、失敗或逾時後自動釋放。",bullets:["不得直接選擇 booted 裝置","不得使用未指定的 adb target","完成或失敗都會 release"],symbol:"play.circle.fill"),
        GuideCopy(title:"公平使用與安全讓位",body:"建立、開機與測試共用同一個 10 分鐘上限；有人等待時，工作必須在安全切點完成並回到隊尾。",bullets:["總佔用上限包含開機","續租次數有限","保留私人環境資料，不關閉別人的裝置"],symbol:"person.2.fill"),
