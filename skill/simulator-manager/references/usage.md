...
New installs use the warm shared FIFO pool. Devices keep apps/data across borrowers, so tests must isolate accounts/records and target only assigned IDs. Pass the saved label and canonical absolute `--project` explicitly on every acquire/run. Omitting the project uses the current directory rather than session registration. For same-platform multiplayer tests, prepare N devices during a drained window with `sim-manager setup ios|android --count N`, then use `run ios|android --count N` or MCP `device_count: N`. The group is admitted all at once and releases every device. Optional Dynamic mode uses fixed private environments per saved label + project + platform, lazily created with capacity reserved before SDK work. Private environment data persists across leases/idle shutdown and is never lent to another session. `max_environments` bounds private assignment count; failed creation remains quarantined.

Optional private Dynamic mode steps through Dynamic, Constrained, Draining and Traditional under host pressure. The default shared mode starts with static FIFO resources and bounds group admissions under pressure. Do not override pressure or change shared config during work. Stage reductions affect new admissions; active work finishes/yields safely. The watcher retires at most one idle private VM per sample, only with exact identity/provenance and a transactionally reserved stopping row. Release itself does not shut down a VM. Shared devices remain running for reuse.
