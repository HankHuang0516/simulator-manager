...
Warm reuse: matching pending requests protect their running private environment. A queue or degraded stage alone does not trigger shutdown. A session releases its lease without powering off the assigned simulator/emulator; never put `simctl shutdown`, `adb emu kill`, emulator-close actions or shutdown-all commands in normal completion or cleanup paths. Maintenance may retire unleased VMs for actual capacity needs, critical telemetry or the configured idle timeout; retain data and let the manager control retirement. Never hold/renew a lease merely to keep a VM warm.
